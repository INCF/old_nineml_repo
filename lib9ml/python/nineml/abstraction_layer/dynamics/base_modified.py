...
class KineticsDynamics(BaseALObject):
    ...
    defining_attributes = ('_regimes', '_aliases', '_state_variables, _KineticState')

    def __init__(self, regimes=None, aliases=None, state_variables=None, kinetic_state=None
                 constants=None, piecewises=None, randomvariables=None):
        """Dynamics object constructor
        ...
        assert_no_duplicates(a.lhs for a in aliases)
        assert_no_duplicates(s.name for s in state_variables)
        assert_no_duplicates(s.name for s in kinetic_state)
...
class KineticsDynamicsClass(ComponentClass, _NamespaceMixin):
