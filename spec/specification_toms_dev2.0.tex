...
\newcommand{\PortConnection}{\defRef{\textbf{\class{PortConnection}}\xspace}{sec:PortConnection}}
...
\noindent
We would like to thank the former INCF NineML Task Force members for their contributions to the text and the concepts presented in this document. 
...
\draftnote{This would be a bit nicer if we split the user layer into composition and parameterisation layers as then \Prototype{}s could be in the parameterisation layer and not have any \PortConnection{}s, which don't really make sense in a prototype.}
...
\subsection{PortConnection}
\label{sec:PortConnection}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{PortConnection Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule \relax
    port & *Port{}@name & yes\\
...
The \PortConnection element specifies a receive port which will receive a connection from a port of a remote component or container. Depending on the type of the \Component{}\withelem{}\ComponentClass the source of remote port to be connected can vary and are specified by different ``port source'' objects, e.g. \FromSource, \FromDestination and \FromSynapse  in \Projection containers.

\subsubsection{Port attribute}
Each \PortConnection element requires a \textit{port} attribute. This should refer to the name of a port in the class definition of the component.
...
\SubComponent elements can refer to any \ComponentClass\withelem\Dynamics class. They elements are explicitly connected to other \SubComponent elements via ports specified by \PortConnection\withelem{}\FromSister elements. Apart from accepting \PortConnection elements, \SubComponent elements are the defined the same as \Component elements.
...
\FromDistal elements without a component attribute accept ports from any compartments that are connected to it (assumes ports have the same name in different domains). With a name attribute they are only connected if the compartment is of the corresponding domain. \PortConnection elements with \FromDistal child elements must be able to handle multiple port connections, i.e. they must be either \EventReceivePort, \AnalogReducePort or \AnalogArrayPort elements.
