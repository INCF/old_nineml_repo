...
It can be seen from the above that what is required is a program that translates the contents of various NMODL blocks into appropriate NineML formatting for the equivalent NineML class and back again. 

%Questions. How many ports can a kinetic scheme have? Would a scheme that had two reactants have 4 ports. 1 send and 1 recieve port per ion channel concentration. It depends if I am modelling concentration of reactants, or number of ion channels in open or closed state.

\begin{lstlisting}
NMODL           NineML
compiledCode    Component Class Instance (Component)
PARAMETER  <->  Parameter
  ?        <->  Ports
STATE      <->  StateVariable.
KINETICS   <->  TimeDerivative
  ?        <->  Aliases
  
\end{lstlisting}

So far many of these functions PARAMETER, PORTS, and Aliases and STATE have been been implemented in the NMODL import in the NinePyPLine package, all that remains to be done is to translate the kinetic scheme syntax into the syntax required by TimeDerivative.

In order to achieve this mapping on the NineML side a Component Class file must be written in the NineML syntax, this component class will have Parameters, which will be converted from NMDOL syntax to NineML syntax, it will have StateVariables which will similarily be converted, and it will have a NMODL kinetic equation which will be converted into NineML TimeDerivative XML syntax an example is below.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \Parameter & set & yes \\
    \StateVariable & set & yes \\

    \ForwardRate & set & yes\\
    \ReverseRate & set & yes\\
    \Conserve & singleton & yes\\
    \AnalogSendPort & set & yes\\
    \AnalogReceivePort & set & yes\\
...
<NineML xmlns="http://nineml.net/9ML/2.0">
  <ComponentClass name="KineticMechanism">
	  <KineticEquation from='state1' to state2>
	  	<Conserve></Conserve>
...
\begin{lstlisting}[label=code:xmliaf2]
  ...
  <ComponentClass name="Kinetic channels">
	  <KineticEquation from='state1' to state2>
	  	<Conserve></Conserve>
	  	<ForwardRate></ForwardRate>
		<ReverseRate></ReverseRate>
